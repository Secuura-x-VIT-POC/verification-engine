...
def main() -> None:
    ...
    if args.mode == "native":
        payload = extract_native(target)
    elif args.mode == "ocr":
        payload = extract_ocr(target, dpi=args.dpi)
    else:
        payload = extract_hybrid(target, dpi=args.dpi)
    print(json.dumps(payload))


def extract_native(path: Path) -> dict:
...
def extract_ocr(path: Path, dpi: int = 300) -> dict:
    document = fitz.open(str(path))
    spatial_map = []
    page_texts = []
    total_chars = 0
    ...
    for page_index, page in enumerate(document, start=1):
        page_payload = _extract_ocr_page(page, page_index, scale, dpi)
...
def extract_hybrid(path: Path, dpi: int = 300) -> dict:
    native_payload = extract_native(path)
    document = fitz.open(str(path))
    combined_tokens = []
    page_texts = []
    page_stats = []
    ...
    for page_index, page in enumerate(document, start=1):
        ...
        if page_word_count < PAGE_WORD_THRESHOLD or page_character_density < PAGE_DENSITY_THRESHOLD:
            ...
            page_stats.append(
                {
                    "page": page_index,
                    "word_count": len(page_payload["tokens"]),
                    "character_density": round(page_payload["character_density"], 4),
...
def _extract_ocr_page(page: fitz.Page, page_index: int, scale: float, dpi: int) -> dict:
