...
def generate_cached_results():
    """
    Processes all PDFs in the samples directory and saves the Canonical JSON output.
    """
    ...
    for filename in pdf_files:
        ...
        # Save exact JSON contract
        cache_path = os.path.join(CACHE_DIR, f"{filename}.json")
        with open(cache_path, "w", encoding="utf-8") as f:
            f.write(result.model_dump_json(indent=2))
            
        if result.is_successful:
            print(f"  ✓ Success! Cached JSON saved to {cache_path}")
            if result.used_ocr:
