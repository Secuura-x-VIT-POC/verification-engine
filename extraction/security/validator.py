...
def _scan_for_malware(path: Path) -> tuple[str, bool]:
    ...
    sha256 = _sha256(path)
    return "sha256_blocklist", sha256 not in blocked_hashes
...
def _load_blocked_hashes() -> set[str]:
    ...
    try:
        ...
        return set()
    hashes = payload.get("blocked_sha256", [])
    return {str(item).lower() for item in hashes}
    ...
    if isinstance(obj, DictionaryObject):
        for key, value in obj.items():
            ...
            if key_name == "/Subtype" and str(value) == "/FileAttachment":
                return "ATTACHMENT_BLOCKED"
            if key_name == "/Type" and str(value) == "/Filespec":
                if "/EF" in obj or "/F" in obj or "/UF" in obj:
                    return "ATTACHMENT_BLOCKED"
            ...
            nested_reason = _walk_pdf_object(value, visited)
            if nested_reason is not None:
                return nested_reason

    if isinstance(obj, ArrayObject):
        for item in obj:
            nested_reason = _walk_pdf_object(item, visited)
            if nested_reason is not None:
                return nested_reason
