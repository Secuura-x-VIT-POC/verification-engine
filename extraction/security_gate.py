...
BLOCKLIST_PATH = Path(__file__).resolve().parent / "security" / "malware_hash_blocklist.json"
...
def validate_document_intake(file_path: str) -> SafetyReport:
    ...
    minimum_resolution = _estimate_minimum_resolution(path)
    safety_report.min_resolution_dpi = minimum_resolution
    if minimum_resolution < MIN_RESOLUTION_DPI:
    ...
    if isinstance(obj, DictionaryObject):
        for key, value in obj.items():
            ...
            if key_name == "/Type" and str(value) == "/Filespec" and ("/EF" in obj or "/F" in obj or "/UF" in obj):
                return "ATTACHMENT_BLOCKED"
            if key_name == "/S" and str(value) in {"/Launch", "/JavaScript"}:
                return "EMBEDDED_JAVASCRIPT_BLOCKED" if str(value) == "/JavaScript" else "LAUNCH_ACTION_BLOCKED"
...
def _estimate_minimum_resolution(path: Path) -> float:
    ...
    for page in document:
        if page.get_text("words"):
            minimum_resolution = min(minimum_resolution, DEFAULT_TEXT_PAGE_DPI)
            continue
        minimum_resolution = min(minimum_resolution, _estimate_page_image_resolution(document, page.number))
