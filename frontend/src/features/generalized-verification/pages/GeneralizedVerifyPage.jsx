...
export default function GeneralizedVerifyPage({ auth, onLogout }) {
	...
	const workspace = useGeneralizedVerificationWorkspace({
		sessionId,
		token: auth.token,
	});
	...
	useEffect(() => {
		...
		if (
			...
			!selectableIds.includes(selectedCredentialId)
		) {
			setSelectedCredentialId(viewModel.selectedCredentialId);
		}
	}, [
		selectedCredentialId,
		viewModel.auditDetails,
		viewModel.highlightItems,
		...
		<div className="page gv-page">
			<div className="app-header">
				<div>
					...
					<p className="muted">
						Read-only reviewer workspace for document profile, routing, audits,
						and overlays.
					</p>
				</div>
				<div className="header-actions">
					<button
						...
						onClick={() =>
							startTransition(() => navigate(getLegacyVerifyPath(sessionId)))
						}
					>
						Legacy Verify Page
					</button>
			...
			{workspace.error ? (
				<div className="panel">
					...
					<p className="muted">
						This workspace stays read-only and will not retry verification
						automatically. You can return to the legacy verify page if you need
					...
					<div className="gv-workspace-layout">
							...
							<div className="panel">
								<div className="gv-workspace-meta">
									...
									<div>
										...
										<p>
											{workspace.data.session.filename || "No file available"}
										</p>
									</div>
								</div>
							</div>
						...
						<WorkspaceRightSidebar
							...
							providerExecutionStatusLabel={
								viewModel.providerExecutionStatusLabel
							}
							providerExecutionStatus={workspace.data.providerExecutionStatus}
