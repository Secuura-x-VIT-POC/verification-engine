...
function createFallbackAuditDetail(credential, plan) {
	...
	const auditStatus = credential.requires_verification
		? "UNVERIFIED"
		: "NOT_APPLICABLE";
	...
	return {
		...
		verifierLabel:
			...
			task?.verifier_label ||
			(credential.requires_verification
				? "Verifier pending"
				: "Not applicable"),
		auditStatus,
		outcomeColor,
...
function buildExecutionInfo(bundle) {
	...
	return {
		...
		preferredProviderKey:
			bestResult.preferred_provider_key ||
			rawSummary.preferred_provider_key ||
			null,
		preferredProviderLabel:
			bestResult.preferred_provider_label ||
			rawSummary.preferred_provider_label ||
			labelFromKey(
				bestResult.preferred_provider_key || rawSummary.preferred_provider_key,
			),
		plannedProviderKey:
			bestResult.planned_provider_key ||
			rawSummary.planned_provider_key ||
			null,
		plannedProviderLabel:
			bestResult.planned_provider_label ||
			rawSummary.planned_provider_label ||
			labelFromKey(
				bestResult.planned_provider_key || rawSummary.planned_provider_key,
			),
		executedProviderKey:
			bestResult.executed_provider_key ||
			rawSummary.executed_provider_key ||
			rawSummary.provider_key ||
			null,
		...
		executionMode:
			bestResult.execution_mode || rawSummary.execution_mode || null,
		fallbackReason:
			bestResult.fallback_reason || rawSummary.fallback_reason || null,
		providerKey:
			bestResult.executed_provider_key ||
			rawSummary.executed_provider_key ||
			rawSummary.provider_key ||
		...
		providerExecutionEnvironmentLabel:
			rawSummary.provider_execution_environment_label || null,
		providerTransitionNotes: Array.isArray(rawSummary.provider_transition_notes)
			? rawSummary.provider_transition_notes
			: [],
		providerIsMockResult: Boolean(
			rawSummary.provider_is_mock_result ?? bestResult.is_mock_result,
		),
		providerIsDemoResult: Boolean(
			rawSummary.provider_is_demo_result ?? bestResult.is_demo_result,
		),
		providerIsLiveResult: Boolean(
			rawSummary.provider_is_live_result ?? bestResult.is_live_result,
		),
	};
}

function createAgentCandidateLookup(candidateCollection) {
	const candidates = Array.isArray(candidateCollection?.candidates)
		? candidateCollection.candidates
		: [];
	return candidates.reduce((lookup, candidate) => {
		const fieldIds = Array.isArray(candidate.grouped_field_ids)
			? candidate.grouped_field_ids
			: [];
...
function createAgentRouteLookup(routeCollection) {
	const recommendations = Array.isArray(routeCollection?.recommendations)
		? routeCollection.recommendations
		: [];
...
function buildRouteProviderInfo(routeDecision, task) {
	...
	const preferredProviderKey =
		routeDecision?.preferred_provider_key ||
		inputPayload.preferred_provider_key ||
		null;
	const preferredProviderLabel =
		routeDecision?.preferred_provider_label ||
		inputPayload.preferred_provider_label ||
		labelFromKey(preferredProviderKey);
	const plannedProviderKey =
		routeDecision?.planned_provider_key ||
		inputPayload.planned_provider_key ||
		null;
	const plannedProviderLabel =
		routeDecision?.planned_provider_label ||
		inputPayload.planned_provider_label ||
		labelFromKey(plannedProviderKey);
	const plannedExecutionMode =
		routeDecision?.planned_execution_mode ||
		inputPayload.planned_execution_mode ||
		null;
	const fallbackReason =
		routeDecision?.fallback_reason || inputPayload.fallback_reason || null;

	let routeDispositionLabel = null;
	let routeDispositionMessage = null;

	if (
		preferredProviderKey === "entra_verified_id" &&
		plannedProviderKey === "entra_verified_id"
	) {
		...
		routeDispositionMessage =
			"No external provider is enabled for this route, so the bounded local mock path is planned.";
		...
		routeDispositionMessage =
			"No executable provider path is available for this credential.";
	}
...
function buildAgentInfo(credentialId, candidateLookup, routeLookup) {
	...
	return {
		...
		recommendedVerifierLabel: labelFromVerifierKey(
...
export function buildAuditDetailViewModels(
	...
	const credentials = Array.isArray(credentialsResponse.credentials)
		? credentialsResponse.credentials
		: [];
	const audits = Array.isArray(auditCollection.audits)
		? auditCollection.audits
		: [];
	const auditLookup = createMapByCredentialId(audits);
	const { routeByCredentialId, taskByCredentialId } =
		createRouteLookup(planResponse);
	...
	return credentials.map((credential) => {
		...
		return {
			...
			normalizedValue:
				audit.normalized_value || credential.normalized_value || null,
			category: credential.category,
			verifierLabel:
			...
			...routeProviderInfo,
			agentAssisted:
				Boolean(agentInfo) || Boolean(extractAgentExplanation(audit.evidence)),
			agentInfo,
	...
	return credentialsResponse.credentials.map((credential) => {
		...
		return {
			...
			providerTechnicalStatus:
				detail?.execution?.providerTechnicalStatus || null,
			providerFallbackUsed: detail?.execution?.providerFallbackUsed || false,
			providerFallbackReason:
				detail?.execution?.fallbackReason || detail?.fallbackReason || null,
			...
			providerOperatingModeLabel:
				detail?.execution?.providerOperatingModeLabel || null,
			...
			providerExecutionEnvironmentLabel:
				detail?.execution?.providerExecutionEnvironmentLabel || null,
			...
			agentRecommendedVerifierLabel:
				detail?.agentInfo?.recommendedVerifierLabel || null,
			agentRouteReason: detail?.agentInfo?.routeReason || null,
			agentManualReviewRecommended:
				detail?.agentInfo?.manualReviewRecommended || false,
		};
	});
}
...
export function buildHighlightItems(
	...
	return credentialsResponse.credentials
		.map((credential) => {
			...
			const detail =
				detailLookup[credential.credential_id] ||
				createFallbackAuditDetail(credential, planResponse);
			return {
				credentialId: credential.credential_id,
				label: credential.label,
...
export function buildTaskExecutionSummary(taskResults, executionStatus) {
	const results = Array.isArray(taskResults?.results)
		? taskResults.results
		: [];
	const executionState =
		executionStatus?.verification_execution_status || "NOT_STARTED";
	...
	return {
		...
		counts,
		verifierKeysUsed: Array.from(
			new Set(results.map((result) => result.verifier_key).filter(Boolean)),
		).sort(),
	};
}

export function buildProviderExecutionSummary(
	providerExecutionTraces,
	providerExecutionStatus,
	providerCapabilities,
) {
	const traces = Array.isArray(providerExecutionTraces?.traces)
		? providerExecutionTraces.traces
		: [];
	const capabilities = Array.isArray(providerCapabilities?.capabilities)
		? providerCapabilities.capabilities
		: [];
	const entraCapability =
		capabilities.find(
			(capability) => capability.provider_key === "entra_verified_id",
		) || null;
	const enabledCapabilities = capabilities.filter(
		(capability) => capability.enabled,
	);
	const operatingMode =
		providerExecutionStatus?.provider_operating_mode || "LIVE_DISABLED";
	return {
		overallStatus:
			providerExecutionStatus?.provider_execution_status || "NOT_STARTED",
		overallLabel: buildProviderExecutionStatusLabel(
		...
		enabledProviders: enabledCapabilities.map(
			(capability) => capability.provider_label,
		),
		...
		primaryTrustRailLabel:
			entraCapability?.provider_label || "Microsoft Entra Verified ID",
		primaryTrustRailEnabled: Boolean(entraCapability?.enabled),
		executionEnvironmentLabel:
			providerExecutionStatus?.execution_environment_label || null,
		demoProfileKey: providerExecutionStatus?.demo_profile_key || null,
		providerTransitionNotes:
			providerExecutionStatus?.provider_transition_notes || [],
		liveProviderEnabled: Boolean(
			providerExecutionStatus?.live_provider_enabled,
		),
		preferredProviderRail:
			providerExecutionStatus?.preferred_provider_rail || "entra_verified_id",
		fallbackPolicy:
			providerExecutionStatus?.fallback_policy ||
			"SUPPLEMENTARY_THEN_LOCAL_MOCK",
		manualReviewPolicy:
			providerExecutionStatus?.manual_review_policy ||
			"RECOMMEND_ON_UNCERTAINTY",
	};
}
...
export function buildAgentUnderstandingSummary(understanding, runStatus) {
	return {
		...
		reasoningSummary:
			...
			"No agent-assisted document understanding is available.",
...
export function getAuditDetailByCredentialId(auditDetails, credentialId) {
	...
	return (
		auditDetails.find((detail) => detail.credentialId === credentialId) ||
		auditDetails[0] ||
		null
	);
}
