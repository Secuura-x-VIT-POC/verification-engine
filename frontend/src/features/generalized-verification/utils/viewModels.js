...
export function buildAuditDetailViewModels(credentialsResponse, auditCollection, planResponse, bundleCollection) {
  ...
  return credentials.map((credential) => {
    ...
    if (!audit && !bundle) {
      return createFallbackAuditDetail(credential, planResponse);
    }
...
export function buildAnalysisRows(credentialsResponse, planResponse, auditCollection, bundleCollection) {
  const details = buildAuditDetailViewModels(credentialsResponse, auditCollection, planResponse, bundleCollection);
...
export function buildHighlightItems(credentialsResponse, auditCollection, planResponse, bundleCollection) {
...
export function buildWorkspaceViewModel({
  ...
  const auditDetails = buildAuditDetailViewModels(credentials, credentialAudits, verificationPlan, credentialBundles);
  const analysisRows = buildAnalysisRows(credentials, verificationPlan, credentialAudits, credentialBundles);
  const highlightItems = buildHighlightItems(credentials, credentialAudits, verificationPlan, credentialBundles);
