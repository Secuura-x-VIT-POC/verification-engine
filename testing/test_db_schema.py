...
class DatabaseSchemaSyncTests(unittest.TestCase):
    def test_sync_existing_schema_adds_missing_session_columns(self):
        ...
        self.assertIn("document_profile_payload", columns)
        self.assertIn("generalized_credentials_payload", columns)
        self.assertIn("verification_plan_payload", columns)
        self.assertIn("verification_task_results_payload", columns)
        self.assertIn("credential_verification_bundles_payload", columns)
        self.assertIn("verification_execution_summary_payload", columns)
        self.assertIn("credential_audits_payload", columns)
        self.assertIn("verification_summary_payload", columns)
        self.assertIn("generalized_analysis_status", columns)
        self.assertIn("generalized_analysis_error", columns)
        self.assertIn("agent_document_understanding_payload", columns)
        self.assertIn("agent_credential_candidates_payload", columns)
        self.assertIn("agent_route_recommendations_payload", columns)
        self.assertIn("agent_explanations_payload", columns)
        self.assertIn("agent_run_summary_payload", columns)
        self.assertIn("agent_run_status", columns)
        self.assertIn("agent_run_error", columns)
